...
import cope.cosmos.client.clickgui.ethius.element.DropDownMenu;
...
public class EnumElement extends SimpleElement {
    ...
    public EnumElement(Setting<Enum<?>> s) {
        ...
        menu = new DropDownMenu(Arrays.stream(s.getValue().getDeclaringClass().getEnumConstants()).map(e -> new SimpleElement().withOnClick((el, f) -> s.setValue(e)).withText(() -> StringUtils.capitalise(e.toString().toLowerCase(Locale.ROOT))).withTooltip("Set " + s.name + " to " + e.toString()).autoWidth(2f).withBackground(() -> e.equals(s.getValue()) ? 0xff232329 : 0xff17171D).withTextAlignment(TextAlignment.LEFT)).collect(Collectors.toList()));
