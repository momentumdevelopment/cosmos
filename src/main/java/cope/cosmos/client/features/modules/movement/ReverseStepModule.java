...
public class ReverseStepModule extends Module {
    public static ReverseStepModule INSTANCE;
    ...
    public void onUpdate() {
        ...
        if (packetLock > 0) {
            ...
            if (mc.getConnection() != null) {
                ...
                while (!packetQueue.isEmpty() && packetCount <= 4) {
                    ((INetworkManager) mc.getConnection().getNetworkManager()).hookDispatchPacket(packetQueue.poll(), null);
                    packetCount++;
                }
            ...
            if (mc.player.fallDistance > 0 && (fallTicks > 0 && fallTicks < 10)) {
                ...
                switch (mode.getValue()) {
                    ...
                    case PACKET:
                        ...
                        if (strictTimer.passedTime(1, Timer.Format.SECONDS)) {

                            // send packets to simulate falling
                            if (fallHeight > 0.5) {
                                if (mc.getConnection() != null) {
                                    ((INetworkManager) mc.getConnection().getNetworkManager()).hookDispatchPacket(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 0.07840000152, mc.player.posZ, false), null);
                                    ((INetworkManager) mc.getConnection().getNetworkManager()).hookDispatchPacket(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 0.23363200604, mc.player.posZ, false), null);
                                    ((INetworkManager) mc.getConnection().getNetworkManager()).hookDispatchPacket(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 0.46415937495, mc.player.posZ, false), null);
                                    ((INetworkManager) mc.getConnection().getNetworkManager()).hookDispatchPacket(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 0.76847620241, mc.player.posZ, false), null);
                                }

                                if (fallHeight >= 1.5) {
                                    packetQueue.add(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 1.14510670065, mc.player.posZ, false));
                                    packetQueue.add(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 1.59260459764, mc.player.posZ, false));
                                    packetLock = 1;

                                    if (fallHeight >= 2.5) {
                                        packetQueue.add(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 2.10955254674, mc.player.posZ, false));
                                        packetQueue.add(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 2.69456154825, mc.player.posZ, false));

                                        if (fallHeight >= 3.5) {
                                            packetQueue.add(new CPacketPlayer.Position(mc.player.posX, mc.player.posY - 3.34627038241, mc.player.posZ, false));
                                            packetLock = 2;
                                        }
                                    }
                                }
    ...
    public void onPacketSend(PacketEvent.PacketSendEvent event) {
        ...
        if (event.getPacket() instanceof CPacketPlayer) {
            ...
            // don't send vanilla packets when packet locked
            if (packetLock > 0) {
