 ...
 * @since 02/02/2022
 */
public class BindFeature extends SettingFeature<AtomicInteger> {
    ...
    public BindFeature(ModuleFeature moduleFeature, Setting<AtomicInteger> setting) {
    ...
    public void drawFeature() {
        ...
        glScaled(0.55, 0.55, 0.55); {
            ...
            float scaledWidth = (getModuleFeature().getCategoryFrameFeature().getPosition().x + getModuleFeature().getCategoryFrameFeature().getWidth() - (FontUtil.getStringWidth(binding ? "Listening ..." : Keyboard.getKeyName(getModuleFeature().getModule().getKey())) * 0.55F) - 3) * 1.81818181F;

            // setting name
            FontUtil.drawStringWithShadow(getSetting().getName(), scaledX, scaledY, -1);

            // bind value
            FontUtil.drawStringWithShadow(binding ? "Listening ..." : Keyboard.getKeyName(getModuleFeature().getModule().getKey()), scaledWidth, scaledY, -1);
        }
        ...
        if (binding) {
            if (in != -1 && in != Keyboard.KEY_ESCAPE) {
                ...
                if (in == Keyboard.KEY_DELETE) {
                    ...
                    getModuleFeature().getModule().setKey(in);
                }
